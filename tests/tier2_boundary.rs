...
fn test_electrocute_boundary_rapid_hits() {
    ...
    assert!(
        trigger_times.len() <= 3,
        "Electrocute should not trigger more than 3 times in 60s (triggered {} times)",
        trigger_times.len()
    );
}
...
fn test_electrocute_boundary_cooldown_limit() {
    ...
    if trigger_times.len() > 1 {
        ...
        assert!(
            diff >= 20000,
            "Second trigger should be at least 20s after the first, but was {} ms",
            diff
        );
    }
}
...
fn test_electrocute_boundary_other_runes() {
    ...
    assert!(
        !has_electrocute,
        "Electrocute should not trigger when not equipped"
    );
}
...
fn test_pta_boundary_decay_no_trigger() {
    ...
    assert!(
        !triggered,
        "PTA should not trigger when attacks are spaced 5s apart (decay window is 4s)"
    );
}
...
fn test_pta_boundary_no_re_application() {
    ...
    let events = parse_gantt_events(&stdout, "Garen");
    let app_times: Vec<u64> = events
        .iter()
        .filter_map(|e| match e {
            GanttEvent::BuffApply { buff_name, time_ms }
                if buff_name.to_lowercase().contains("press the attack")
                    || buff_name.to_lowercase().contains("pta") =>
            {
                Some(*time_ms)
            }
    ...
    if app_times.len() > 1 {
        ...
        assert!(
            diff >= 6000,
            "PTA exposure debuff should not be re-applied or refreshed before it expires (6s)"
        );
    }
}
...
fn test_pta_boundary_cooldown() {
    ...
    if trigger_times.len() > 1 {
        ...
        assert!(
            diff >= 6000,
            "PTA activation spacing must respect the 6s cooldown"
        );
    }
}
...
fn test_pta_boundary_other_runes() {
    ...
    let breakdown = parse_breakdown(&stdout, "Garen");
    let has_pta = breakdown
        .keys()
        .any(|k| k.to_lowercase().contains("press the attack") || k.to_lowercase().contains("pta"));
    assert!(
        !has_pta,
        "PTA should not trigger when Conqueror is equipped"
    );
}
