...
fn test_grasp_of_the_undying_integration() {
    ...
    let stdout = run_with_apl("Garen", "Dummy", Some("grasp_of_the_undying"), None, apl, &["--html-out", "temp_grasp.html"]);
    
    let breakdown = parse_breakdown(&stdout, "Garen");
    assert!(breakdown.contains_key("GraspOfTheUndying"), "Should deal Grasp of the Undying magic damage");
...
fn test_triumph_integration() {
    ...
    let has_death = events.iter().any(|e| e["type"] == "death" && e["source"] == "Darius");
    ...
    assert!(has_triumph_heal, "Ahri should receive Triumph heal on takedown");
}
...
fn test_legend_alacrity_integration() {
    ...
    let stdout_no_alacrity = run_with_apl("Garen", "Dummy", Some("conqueror,triumph,bone_plating"), None, apl, &[]);
    let events_no_alacrity = parse_gantt_events(&stdout_no_alacrity, "Garen");
    
    let stdout_alacrity = run_with_apl("Garen", "Dummy", Some("conqueror,triumph,legend_alacrity,bone_plating"), None, apl, &[]);
    let events_alacrity = parse_gantt_events(&stdout_alacrity, "Garen");

    let aa_times_no_alacrity: Vec<u64> = events_no_alacrity.iter().filter_map(|e| match e {
        GanttEvent::Cast { ability, time_ms } if ability == "AutoAttack" => Some(*time_ms),
        _ => None,
    }).collect();

    let aa_times_alacrity: Vec<u64> = events_alacrity.iter().filter_map(|e| match e {
        GanttEvent::Cast { ability, time_ms } if ability == "AutoAttack" => Some(*time_ms),
        _ => None,
    }).collect();
...
fn test_last_stand_integration() {
    ...
    let _stdout = run_with_apl(
        ...
        None,
        apl_garen,
        &["--apl-b", apl_darius_filename, "--html-out", "temp_last_stand.html"]
    );
    ...
    let garen_aa_damages: Vec<(f64, f64)> = events.iter().filter_map(|e| {
        if e["type"] == "damage" && e["source"] == "Garen" && e["ability"] == "AutoAttack" {
            Some((e["time"].as_f64().unwrap_or(0.0), e["amount"].as_f64().unwrap_or(0.0)))
        } else {
            None
        }
    }).collect();
...
fn test_bone_plating_integration() {
    ...
    let count = stdout.lines().filter(|line| line.contains("Garen's Bone Plating blocked 60.0 damage")).count();
...
fn test_overgrowth_integration() {
    ...
    let stdout = run_with_apl("Garen", "Dummy", Some("grasp_of_the_undying,overgrowth"), None, apl, &["--html-out", "temp_overgrowth.html"]);
    
    let html_content = std::fs::read_to_string("temp_overgrowth.html").unwrap();
    let _ = std::fs::remove_file("temp_overgrowth.html");

    assert!(html_content.contains("overgrowth"), "Overgrowth should be equipped in HTML report");
    assert!(stdout.contains("Champion: \"Garen\""), "Simulation should run successfully");
}
...
fn test_infinity_edge_integration() {
    ...
    let _stdout_no_ie = run_with_apl("Garen", "Dummy", None, None, apl_no_ie, &["--html-out", "temp_no_ie.html"]);
    ...
    let dmg_no_ie = events_no_ie.iter().find(|e| e["type"] == "damage" && e["source"] == "Garen" && e["ability"] == "AutoAttack")
        .and_then(|e| e["amount"].as_f64()).unwrap_or(0.0);

    // 2. With IE (IE + 4 Phantom Dancers to get 100% crit chance)
    let apl_ie = "items=3031,3046,3046,3046,3046\nactions+=/AA,if=time<0.1\n";
    let _stdout_ie = run_with_apl("Garen", "Dummy", None, None, apl_ie, &["--html-out", "temp_ie.html"]);
    ...
    let dmg_ie = events_ie.iter().find(|e| e["type"] == "damage" && e["source"] == "Garen" && e["ability"] == "AutoAttack")
        .and_then(|e| e["amount"].as_f64()).unwrap_or(0.0);
...
fn test_mortal_reminder_integration() {
    ...
    assert!(stdout.contains("Grievous Wounds"), "Physical damage should apply Grievous Wounds");
}
...
fn test_phantom_dancer_integration() {
    ...
    assert!(stdout.contains("Spectral Waltz"), "Basic attacks should trigger Spectral Waltz");
}
...
fn test_ahri_integration() {
    let apl = "actions+=/E\nactions+=/Q\nactions+=/W\nactions+=/R\n";
    let _stdout = run_with_apl("Ahri", "Darius", None, None, apl, &["--html-out", "temp_ahri.html"]);
    
    let html_content = std::fs::read_to_string("temp_ahri.html").unwrap();
    let _ = std::fs::remove_file("temp_ahri.html");
    let events = parse_html_events(&html_content);

    let has_q = events.iter().any(|e| e["type"] == "damage" && e["source"] == "Ahri" && e["ability"] == "Q");
    let has_w = events.iter().any(|e| e["type"] == "damage" && e["source"] == "Ahri" && e["ability"] == "W");
    let has_e = events.iter().any(|e| e["type"] == "damage" && e["source"] == "Ahri" && e["ability"] == "E");
    let has_r = events.iter().any(|e| e["type"] == "damage" && e["source"] == "Ahri" && e["ability"] == "R");
    ...
    let has_charm = events.iter().any(|e| e["type"] == "buff_apply" && e["target"] == "Darius" && e["buff_name"] == "매혹");
...
fn test_zed_integration() {
    ...
    let _stdout = run_with_apl("Zed", "Garen", None, None, apl_zed, &["--html-out", "temp_zed.html"]);

    let html_content = std::fs::read_to_string("temp_zed.html").unwrap();
    let _ = std::fs::remove_file("temp_zed.html");
    let events = parse_html_events(&html_content);

    let zed_damages: Vec<_> = events.iter().filter(|e| e["type"] == "damage" && e["source"] == "Zed").collect();
    ...
    assert!(has_mimic, "Zed shadow should mimic abilities (resulting in concurrent damage hits)");

    let has_mark = events.iter().any(|e| e["type"] == "buff_apply" && e["target"] == "Garen" && e["buff_name"].as_str().is_some_and(|s| s.contains("죽음의 표식") || s.contains("Death Mark")));
...
fn test_jinx_integration() {
    ...
    let _stdout = run_with_apl("Jinx", "Ahri", None, None, apl_jinx, &["--html-out", "temp_jinx.html"]);

    let html_content = std::fs::read_to_string("temp_jinx.html").unwrap();
    let _ = std::fs::remove_file("temp_jinx.html");
    let events = parse_html_events(&html_content);

    let has_powpow_buff = events.iter().any(|e| e["type"] == "buff_apply" && e["target"] == "Jinx" && e["buff_name"].as_str().is_some_and(|s| s.contains("Pow-Pow Stacks") || s.contains("신난다!")));
