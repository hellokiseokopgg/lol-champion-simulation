...
pub fn parse_total_damage(stdout: &str, champion: &str) -> Option<f64> {
    ...
    for line in stdout.lines() {
        let line = line.trim();
        if let Some(stripped) = line.strip_prefix("Champion:") {
            let name = stripped.trim().trim_matches('"');
            in_champion = name == champion;
            continue;
        }
        if in_champion {
            if let Some(stripped) = line.strip_prefix("Total Damage:") {
...
pub fn parse_gantt_events(stdout: &str, champion: &str) -> Vec<GanttEvent> {
    ...
    for line in stdout.lines() {
        ...
        if let Some(stripped) = line.strip_prefix("section ") {
            let section_name = stripped.trim();
        ...
        if let Some(stripped) = line.strip_prefix("Cast ") {
            ...
            if parts.len() == 2 {
                ...
                if let Ok(start_ms) = times[0].trim().parse::<u64>() {
                    events.push(GanttEvent::Cast {
                        ability,
                        time_ms: start_ms,
                    });
                }
            }
        } else if let Some(stripped) = line.strip_prefix("Dmg ") {
            let parts: Vec<&str> = stripped.split(':').collect();
            if parts.len() == 2 {
                let ability = parts[0].trim().to_string();
                let times: Vec<&str> = parts[1].split(',').collect();
                if let Ok(start_ms) = times[0].trim().parse::<u64>() {
                    events.push(GanttEvent::Damage {
                        ability,
                        time_ms: start_ms,
                    });
                }
            }
        } else if let Some(stripped) = line.strip_prefix("Effect ") {
            let parts: Vec<&str> = stripped.split(':').collect();
            if parts.len() == 2 {
                let raw_buff = parts[0].trim();
                let times: Vec<&str> = parts[1].split(',').collect();
                if let Ok(start_ms) = times[0].trim().parse::<u64>() {
                    if let Some(stripped_buff) = raw_buff.strip_suffix(" 만료") {
                        let buff_name = stripped_buff.trim().to_string();

                        events.push(GanttEvent::BuffExpire {
                            buff_name,
                            time_ms: start_ms,
                        });
                    } else {
                        events.push(GanttEvent::BuffApply {
                            buff_name: raw_buff.to_string(),
                            time_ms: start_ms,
                        });
                    }
                }
